...
FROM
    ...
    acadgeral__aluno ON acadgeral__aluno.pes_id = pessoa_fisica.pes_id	
        INNER JOIN 
    documento__grupo_regra dgr ON JSON_CONTAINS(pessoa.pes_docgruporegra_id, CAST(dgr.docgruporegra_id as JSON), '$')
        INNER JOIN
    documento__tipo ON documento__tipo.tdoc_id = dgr.tdoc_id
       LEFT JOIN
    documento__pessoa ON documento__pessoa.pes_id = pessoa.pes_id AND documento__pessoa.docgruporegra_id = dgr.docgruporegra_id
